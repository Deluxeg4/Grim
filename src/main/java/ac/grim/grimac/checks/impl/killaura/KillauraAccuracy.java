...
public class KillauraAccuracy extends Check implements PacketCheck {
    ...
    public void onReload(ConfigManager config) {
        this.maxAccuracy = Math.min(100, config.getIntElse(getConfigName() + ".maxAccuracy", 95));
        this.sampleSize = config.getIntElse(getConfigName() + ".sampleSize", 25);
        this.minDistance = config.getDoubleElse(getConfigName() + ".minDistance", 0.7);
