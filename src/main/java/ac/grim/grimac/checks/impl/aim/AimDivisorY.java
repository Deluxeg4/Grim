...
public class AimDivisorY extends Check implements RotationCheck {
    ...
    public void onReload(ConfigManager config) {
        sampleSize = config.getIntElse(getConfigName() + ".sampleSize", 25);
        maxInvalidRows = config.getDoubleElse(getConfigName() + ".maxInvalidRows", 2);  //How many independent rows are allowed in the sample size
        minRowLength = config.getDoubleElse(getConfigName() + ".minRowLength", 3); //The minimum length
        minAverageRot = config.getDoubleElse(getConfigName() + ".minAverageRot", 0.4D);
